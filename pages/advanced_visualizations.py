...
with tab1:
    ...
    selected_layout = st.selectbox("Select Dashboard Layout", layout_options)
    ...
    if st.session_state.saved_visualizations:
        ...
        if selected_config:
            ...
            if st.button(f"Delete Dashboard: {selected_dashboard}"):
                st.session_state.saved_visualizations = [
...
with tab2:
    ...
    if len(categorical_columns) >= 1:
        level1 = st.selectbox("Level 1 (Highest)", categorical_columns)
        
        # Filter remaining columns for level 2
        level2_options = [col for col in categorical_columns if col != level1]
        level2 = st.selectbox("Level 2 (Optional)", ["None"] + level2_options) if level2_options else "None"
        
        # Filter remaining columns for level 3
        if level2 != "None":
            level3_options = [col for col in categorical_columns if col not in [level1, level2]]
            level3 = st.selectbox("Level 3 (Optional)", ["None"] + level3_options) if level3_options else "None"
        else:
            level3 = "None"
        
        # Measure column
        measure = st.selectbox("Measure (Numeric Value)", numeric_columns)
        
        # Aggregation method
        aggregation = st.selectbox("Aggregation Method", ["Sum", "Average", "Count", "Min", "Max"])
        
        # Chart type
        chart_type = st.selectbox("Chart Type", ["Bar Chart", "Treemap", "Sunburst"])
        ...
        if st.button("Generate Drill-Down Visualization"):
        ...
        if len(data) == 0:
            ...
            if display_level:
                ...
                if current_level > 0:
                    if st.button("⬆️ Go Back Up a Level"):
...
with tab3:
    ...
    if len(numeric_columns) < 3:
        st.warning("3D visualizations require at least 3 numeric columns. Your data has only {len(numeric_columns)} numeric columns.")
    else:
        ...
        if viz_type == "3D Scatter":
            x_column = st.selectbox("X-axis Column", numeric_columns, index=0)
            y_column = st.selectbox("Y-axis Column", numeric_columns, index=min(1, len(numeric_columns)-1))
            z_column = st.selectbox("Z-axis Column", numeric_columns, index=min(2, len(numeric_columns)-1))
            
            color_by = st.selectbox("Color By (optional)", ["None"] + categorical_columns + numeric_columns)
            size_by = st.selectbox("Size By (optional)", ["None"] + numeric_columns)
            ...
            x_column = st.selectbox("X-axis Column", numeric_columns, index=0)
            y_column = st.selectbox("Y-axis Column", numeric_columns, index=min(1, len(numeric_columns)-1))
            z_column = st.selectbox("Z-axis Column (Height)", numeric_columns, index=min(2, len(numeric_columns)-1))
            ...
            # Generate visualization
            if st.button("Generate 3D Line Plot"):
                ...
                if len(data) < 3:
                    ...
                    if sequence_column != "Index":
                        ...
                        step_idx = st.slider("Position in sequence", 1, steps, 1)
...
with tab4:
    ...
    if animation_type == "Time Series":
        if not date_columns:
            ...
            # Select time column
            time_column = st.selectbox("Time Column", date_columns)
            ...
            if time_data is not None:
                ...
                # Select metrics
                x_column = st.selectbox("X-axis Column", numeric_columns + categorical_columns, index=0)
                y_column = st.selectbox("Y-axis Column", numeric_columns, index=0)
                
                # Optional color column
                color_column = st.selectbox("Color By (optional)", ["None"] + categorical_columns)
                
                # Animation settings
                animation_speed = st.slider("Animation Speed", 500, 2000, 1000, 100)
                
                # Generate animation
                if st.button("Generate Time Series Animation"):
        ...
        if not categorical_columns:
            ...
            size_column = st.selectbox("Size By (optional)", ["None"] + numeric_columns)
            
            # Animation settings
            animation_speed = st.slider("Animation Speed", 500, 2000, 1000, 100)
            
            # Generate animation
            if st.button("Generate Category Transition Animation"):
                # Create the animated chart
        ...
        value_column = st.selectbox("Value Column (to show growth)", numeric_columns)
        
        # Optional grouping column
        group_column = st.selectbox("Group By (optional)", ["None"] + categorical_columns)
        ...
        # Animation settings
        num_frames = st.slider("Number of Animation Frames", 10, 50, 20)
        animation_speed = st.slider("Animation Speed", 50, 500, 200, 10)
        
        # Generate animation
        if st.button("Generate Cumulative Growth Animation"):
            # Create the animated chart
